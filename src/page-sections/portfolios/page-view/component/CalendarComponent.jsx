...
const CalendarComponent = ({
  ...
  dataStat = {},
  isGlobal = false,
  statGlobal = { profit: 0, win: 0, lose: 0, volume: 0 },
}) => {
  ...
  const groupedData = _.groupBy(
    events,
    (item) =>
      `${item.title}-${moment(item.start).format("DD/MM/YYYY")}-${moment(
        item.end
      ).format("DD/MM/YYYY")}-${item.color}`
  );
  ...
  result?.push({
    ...
    desc: statGlobal?.profit,
    color:
      parseFloat(statGlobal.profit) >= 0
        ? theme.palette.success.main
        : theme.palette.error.main,
  });
  result?.push({
        ...
        <Box sx={{ width: "100%" }}>
          ...
          >
            <Box sx={{ width: downLg ? "100%" : "48%" }}>
              <Box mb={4}>
                <Typography fontWeight={600} ml={2} fontSize={18}>
                  Vol
                </Typography>
              </Box>
              <Box>
                <GrandTotalProfitChart
                  data={result}
                  monthProps={parseInt(moment(currentDate).format("M"))}
                />
              </Box>
            </Box>
            <Box sx={{ width: downLg ? "100%" : "48%" }}>
