...
const CustomToolbar = ({
  ...
  const monthIndex = moment(currentDate).month() + parseInt(1);
  const yearIndex = moment(currentDate).year();
  console.log("month index", monthIndex, yearIndex);
  // const year = moment(label).year();
  // const formattedLabel = `${monthNames[monthIndex]} ${year}`;
  const downLg = useMediaQuery((theme) => theme.breakpoints.down("lg"));
  // loc theo thang hien tai
  ...
  const renderTotalProfit = () => {
    if (isGlobal) {
      if (walletMode === true) {
        return _.sumBy(
          filterByMonthYear(data, monthIndex, yearIndex),
          function (e) {
